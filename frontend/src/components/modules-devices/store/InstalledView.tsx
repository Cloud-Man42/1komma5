  ...
  const load = useCallback(async () => {
    ...
    try {
      const [cat, st] = await Promise.all([
        fetchStoreCatalog({ installed: true, page_size: 100, sort: "name" }),
        fetchStoreStatus(),
      ]);
      setCatalog(cat);
      setStatus(st);
  ...
  return (
    <>
      ...
      <ModuleStoreShell status={status}>
        <h2 className="config-panel-title">Installed modules</h2>
        {error ? <p className="config-error">{error}</p> : null}
        {catalog?.modules.length === 0 ? <p className="muted">No installed modules.</p> : null}
