...
} from "@/components/spa-dashboard/spaDashboardHelpers";
...
export function SpaControlSettingsPanel({ siteSlug }: { siteSlug: string }) {
  ...
  const handleSave = async () => {
    ...
    setMessage(null);
    try {
      const updated = await updateSpaControlConfig(siteSlug, buildSpaControlUpdatePayload(config));
      setConfig(updated);
      ...
      <div className="config-form">
        ...
        <fieldset className="spa-settings-section">
          ...
          <div className="form-grid">
            ...
            {fixedSchedule && fixedScheduleWarningSv(config) ? (
              <div className="spa-fixed-schedule-warn" data-testid="fixed-schedule-warning">
                <p className="form-error">{fixedScheduleWarningSv(config)}</p>
                {(() => {
                  const recommended = recommendedFixedScheduleTimes(config);
                  return (
                    <>
                      <p className="muted">
                        Rekommenderat för Glacier XL:{" "}
                        <strong>
                      ...
                      <button
                        ...
                        onClick={() =>
                          setConfig({
                            ...config,
                            fixed_schedule_start: recommended.start,
                            fixed_schedule_end: recommended.end,
                          })
                        }
                      >
