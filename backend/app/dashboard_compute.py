"""Shared dashboard section builders used by the API and display services."""
...
from app.schemas.dashboard import DashboardAlert, DashboardEvSection, DashboardFreshnessSection, DashboardLiveSection, DashboardOptimizationSection, DashboardPriceSection, DashboardSolarSection, DashboardTodaySection, DashboardVehicleSection
...
async def _compute_today(session: AsyncSession, site, settings: Settings) -> DashboardTodaySection:
    ...
    if settings.financial_aggregates_enabled:
        ...
        if today_row is not None:
            ...
            produced = float(today_row.solar_kwh)
            consumed = float(today_row.consumption_kwh)
            imported = float(today_row.import_kwh)
            exported = float(today_row.export_kwh)
            ...
            if len(live_readings) >= 2:
                totals = integrate_site_energy(live_readings)
