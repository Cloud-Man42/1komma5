...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_energy_balance_snapshots_charger_id", "energy_balance_snapshots", ["charger_id"]
    )
    op.create_index(
        "ix_energy_balance_snapshots_recorded_at", "energy_balance_snapshots", ["recorded_at"]
    )
    ...
    op.add_column(
        "ev_chargers",
        sa.Column("semp_endpoint_registered_at", sa.DateTime(timezone=True), nullable=True),
    )
