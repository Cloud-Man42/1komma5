...
def upgrade() -> None:
    ...
    op.add_column(
        "sites", sa.Column("safety_margin_a", sa.Float(), nullable=False, server_default="2.0")
    )

    op.add_column("ev_chargers", sa.Column("required_energy_kwh", sa.Float(), nullable=True))
    op.add_column(
        "ev_chargers", sa.Column("deadline_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "ev_chargers",
        sa.Column("solar_start_threshold_w", sa.Float(), nullable=False, server_default="1500"),
    )
    op.add_column(
        "ev_chargers",
        sa.Column("solar_stop_threshold_w", sa.Float(), nullable=False, server_default="800"),
    )
    op.add_column(
        "ev_chargers",
        sa.Column("solar_start_delay_seconds", sa.Integer(), nullable=False, server_default="30"),
    )
    op.add_column(
        "ev_chargers",
        sa.Column("solar_stop_delay_seconds", sa.Integer(), nullable=False, server_default="60"),
    )
    op.add_column(
        "ev_chargers", sa.Column("last_charging_action", sa.String(length=32), nullable=True)
    )
    op.add_column(
        "ev_chargers", sa.Column("last_charging_reason", sa.String(length=64), nullable=True)
    )
    op.add_column(
        "ev_chargers", sa.Column("last_charger_error_code", sa.String(length=32), nullable=True)
    )
    ...
    op.execute(
        "UPDATE ev_chargers SET control_source = 'chargeamp' WHERE control_source = 'heartbeat'"
    )
