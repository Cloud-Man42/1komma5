...
class SolarForecastCoordinator:
    ...
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._engine = SolarForecastEngine(
    ...
    async def _refresh_site_if_due(
        ...
        *,
        ...
        if (
            ...
            or not record.installed_peak_power_kw
        ):
            return False
    ...
    async def evaluate_site_observations(
        self, session, site, *, now: datetime | None = None
    ) -> None:
    ...
    async def _evaluate_daily_observations(self, session, site, *, now: datetime) -> None:
        ...
        readings = await reading_repo.list_readings(
            site.id, from_time=since, to_time=now, limit=100000
        )
    ...
    async def _fetch_weather(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "Weather provider failed for site %s, trying cache/fallback", site_config.site_id
            )
    ...
    async def _collect_samples(
        self, session, site, domain, *, now: datetime
        ...
        readings = await reading_repo.list_readings(
            site.id, from_time=since, to_time=now, limit=50000
        )
    ...
    async def _evaluate_site_if_due(self, session, site, *, now: datetime) -> None:
        ...
        readings = await reading_repo.list_readings(
            site.id, from_time=from_ts, to_time=to_ts, limit=1000
        )
