...
class Settings(BaseSettings):
    ...
    solar_forecast_horizon_hours: int = Field(
        default=48, ge=12, le=72, alias="SOLAR_FORECAST_HORIZON_HOURS"
    )
    solar_forecast_refresh_minutes: int = Field(
        default=30, ge=5, alias="SOLAR_FORECAST_REFRESH_MINUTES"
    )
    ...
    solar_forecast_retention_days: int = Field(
        default=14, ge=1, alias="SOLAR_FORECAST_RETENTION_DAYS"
    )
    ...
    open_meteo_timeout_seconds: float = Field(
        default=30.0, ge=5.0, alias="OPEN_METEO_TIMEOUT_SECONDS"
    )

    # Solar forecast v2 calibration thresholds
    solar_forecast_min_samples_preliminary: int = Field(
        default=7, ge=1, alias="SOLAR_FORECAST_MIN_SAMPLES_PRELIMINARY"
    )
    solar_forecast_min_samples_calibrated: int = Field(
        default=30, ge=1, alias="SOLAR_FORECAST_MIN_SAMPLES_CALIBRATED"
    )
    solar_forecast_min_samples_mature: int = Field(
        default=60, ge=1, alias="SOLAR_FORECAST_MIN_SAMPLES_MATURE"
    )
    solar_forecast_rolling_window_days: int = Field(
        default=60, ge=7, alias="SOLAR_FORECAST_ROLLING_WINDOW_DAYS"
    )
    solar_forecast_mape_min_actual_kwh: float = Field(
        default=1.0, ge=0.0, alias="SOLAR_FORECAST_MAPE_MIN_ACTUAL_KWH"
    )
    ...
    solar_forecast_correction_factor_min: float = Field(
        default=0.70, gt=0.0, alias="SOLAR_FORECAST_CORRECTION_FACTOR_MIN"
    )
    solar_forecast_correction_factor_max: float = Field(
        default=1.30, gt=0.0, alias="SOLAR_FORECAST_CORRECTION_FACTOR_MAX"
    )
    solar_forecast_outlier_ratio_min: float = Field(
        default=0.30, ge=0.0, alias="SOLAR_FORECAST_OUTLIER_RATIO_MIN"
    )
    solar_forecast_outlier_ratio_max: float = Field(
        default=1.70, ge=0.0, alias="SOLAR_FORECAST_OUTLIER_RATIO_MAX"
    )
    solar_forecast_correction_ema_alpha: float = Field(
        default=0.15, gt=0.0, le=1.0, alias="SOLAR_FORECAST_CORRECTION_EMA_ALPHA"
    )

    # Sungrow / energy balance / Virtual EVSE (Phase 1)
    sungrow_telemetry_max_age_seconds: float = Field(
        default=60.0, ge=5.0, alias="SUNGROW_TELEMETRY_MAX_AGE_SECONDS"
    )
    max_telemetry_alignment_age_seconds: float = Field(
        default=10.0, ge=1.0, alias="MAX_TELEMETRY_ALIGNMENT_AGE_SECONDS"
    )
    energy_balance_residual_warn_w: float = Field(
        default=500.0, ge=0.0, alias="ENERGY_BALANCE_RESIDUAL_WARN_W"
    )
    double_counting_tolerance_w: float = Field(
        default=800.0, ge=0.0, alias="DOUBLE_COUNTING_TOLERANCE_W"
    )
    virtual_evse_stale_seconds: float = Field(
        default=120.0, ge=30.0, alias="VIRTUAL_EVSE_STALE_SECONDS"
    )

    # Arctic Spa integration
    arctic_spa_enabled: bool = Field(default=False, alias="ARCTIC_SPA_ENABLED")
    arctic_spa_api_base_url: str = Field(
        default="https://api.myarcticspa.com", alias="ARCTIC_SPA_API_BASE_URL"
    )
    arctic_spa_api_key: str = Field(default="", alias="ARCTIC_SPA_API_KEY")
    arctic_spa_poll_interval_seconds: int = Field(
        default=60, ge=15, le=600, alias="ARCTIC_SPA_POLL_INTERVAL_SECONDS"
    )
