...
class ChargingStationResolver:
    ...
    async def resolve(
        ...
        *,
        ...
        if self._cache_repo is not None:
            ...
            if cached is not None and cached.price_model not in {None, "UNKNOWN", "FREE"}:
                ...
                return cached
            if cached is not None and self._status_repo is not None:
    ...
    async def enrich_pricing(self, resolved: ResolvedChargingLocation) -> ResolvedChargingLocation:
        """Fill in operator price when an earlier lookup/cache entry lacked outlet pricing."""
        if resolved.price_model not in {None, "UNKNOWN", "FREE"} or resolved.selected_station is None:
            return resolved
        enrich = getattr(self._provider, "_enrich_missing_pricing", None)
        if enrich is None:
            return resolved
        station = resolved.selected_station
        enriched_candidates = await enrich(
            [station],
            vehicle_lat=station.latitude,
            vehicle_lon=station.longitude,
            max_enrich=1,
        )
        if not enriched_candidates:
            return resolved
        enriched_station = enriched_candidates[0]
        if enriched_station.price_model in {None, "UNKNOWN"}:
            return resolved
        from dataclasses import replace as dc_replace

        return dc_replace(
            resolved,
            price_model=enriched_station.price_model,
            price_value_sek_kwh=enriched_station.price_value_sek_kwh,
            selected_station=enriched_station,
        )
