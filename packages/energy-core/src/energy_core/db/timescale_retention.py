    ...
    for hypertable, interval in RETENTION_POLICIES.items():
        try:
            ...
            await session.execute(
                text("SELECT add_retention_policy(:hypertable, INTERVAL :interval)"),
                {"hypertable": hypertable, "interval": interval},
            )
    ...
    for hypertable, policy in COMPRESSION_POLICIES.items():
        ...
        try:
            if await _compression_enabled(session, hypertable):
                ...
                await session.execute(
                    text(
                        ...
                        "(timescaledb.compress, timescaledb.compress_segmentby = :segmentby)"
                    ),
                    {"segmentby": segmentby},
                )
            ...
            if await _compression_policy_exists(session, hypertable):
                ...
                await session.execute(
                    text("SELECT add_compression_policy(:hypertable, INTERVAL :compress_after)"),
