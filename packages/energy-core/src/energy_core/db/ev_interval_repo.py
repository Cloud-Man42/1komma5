...
from energy_core.db.models import EvChargingIntervalModel


@dataclass(frozen=True, slots=True)
class EvChargingIntervalRecord:
    id: int
    session_id: int
    charger_id: int
    start_time: datetime
...
class EvChargingIntervalRepository:
    def __init__(self, session: AsyncSession) -> None:
        self._session = session

    async def insert(self, **fields) -> EvChargingIntervalRecord:
        row = EvChargingIntervalModel(**fields)
        self._session.add(row)
        await self._session.flush()
        return self._to_record(row)

    async def list_for_session(self, session_id: int) -> list[EvChargingIntervalRecord]:
        rows = await self._session.scalars(
            select(EvChargingIntervalModel)
            .where(EvChargingIntervalModel.session_id == session_id)
            .order_by(EvChargingIntervalModel.start_time)
        )
        return [self._to_record(r) for r in rows]

    @staticmethod
    def _to_record(row: EvChargingIntervalModel) -> EvChargingIntervalRecord:
        return EvChargingIntervalRecord(
            id=row.id,
