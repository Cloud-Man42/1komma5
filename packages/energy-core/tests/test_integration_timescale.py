...
async def test_timescaledb_hypertable_exists():
    ...
    async with engine.connect() as conn:
        result = await conn.execute(
            text("SELECT hypertable_name FROM timescaledb_information.hypertables WHERE hypertable_name = 'energy_readings'")
        )
        row = result.first()
        if row is None:
            pytest.skip("TimescaleDB hypertable not created — run migrations with ENABLE_TIMESCALEDB=true")
